...
enum {
    PIC_MASTER_DATA_PORT = 0x20
    , PIC_MASTER_COMMAND_PORT = 0x21
    , PIC_MASTER_OFFSET = 0x20
    , PIC_SLAVE_DATA_PORT = 0xa0
    , PIC_SLAVE_COMMAND_PORT = 0xa1
...
void pic_initialize(void) {
    ...
    const uint8_t master_data = io_read(PIC_MASTER_DATA_PORT);
    const uint8_t slave_data = io_read(PIC_SLAVE_DATA_PORT);
    ...
    io_write(PIC_MASTER_DATA_PORT, master_data);
    io_write(PIC_SLAVE_DATA_PORT, slave_data);
